...
class WrappedTransactionTest extends TestCase
{
    ...
    public function testDecodeRaw ($expect, $privateKey, $type, $chainId, $nonce, $maxPriorityFeePerGas, $maxFeePerGas, $gasLimit, $to, $value, $data, $accessList) {
        ...
        $rlp = new RLP();
        $toDecode = substr($expect, 2);
        $decoded = $rlp->decode($toDecode);
        ...
        $this->assertSame($decoded[0], $this->padHex($chainId));
        $this->assertSame($decoded[1], hexdec($nonce) !== 0 ? $this->padHex($nonce) : '');
        $this->assertSame($decoded[2], $this->padHex($maxPriorityFeePerGas));
        $this->assertSame($decoded[3], $this->padHex($maxFeePerGas));
        $this->assertSame($decoded[4], $this->padHex($gasLimit));
        $this->assertSame($decoded[5], $this->padHex($to));
        $this->assertSame($decoded[6], $this->padHex($value));
        $this->assertSame($decoded[7], $this->padHex($data));
    ...
    public static function getRaw (): array {
        return [
            ...
            [
                '02f87501028503f5476a008503f5476a0083027f4b941a8c8adfbe1c59e8b58cc0d515f07b7225f51c72882a45907d1bef7c0080c080a09eef0b7f1a5adc894c166ee60008058227bb3880df9d1ac10bdcfd2814aa03f9a028fe9af51a915232947676571ca56e47dabe7f6a5c62e7d61355407ee7ba0975',
                'b2f2698dd7343fa5afc96626dee139cb92e58e5d04e855f4c712727bf198e898', '2', '1', '2', '03f5476a00', '03f5476a00', '027f4b', '1a8c8adfbe1c59e8b58cc0d515f07b7225f51c72', '2a45907d1bef7c00', '', []
            ],
            [
    ...
    public function testBadPrivateKey (): void
    {
        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('Incorrect private key');

        $transaction = new WrappedTransaction(2, 0);
        $transaction->getRaw('');
    }
}
